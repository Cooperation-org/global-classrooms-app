...
export default function ProjectsPage() {
  ...
  const filteredProjects = useMemo(() => {
    ...
    switch (activeTab) {
      case "Joined":
        filtered = filtered.filter(
          (project: any) => project.is_open_for_collaboration
        );
        break;
      case "Collaborative":
        filtered = filtered.filter(
    ...
    if (selectedStatus !== "All Status") {
      filtered = filtered.filter(
        (project: any) => project.status === selectedStatus.toLowerCase()
      );
    }
    ...
    return Array.from(
      new Set(
        projects.flatMap((project: any) =>
          Object.values(project.environmental_themes)
        )
      )
    ).sort() as string[];
  }, [projects]);
  ...
  const uniqueStatuses = useMemo(() => {
    if (!projects) return [];
    return Array.from(
      new Set(projects.map((project: any) => project.status))
    ).sort() as string[];
  }, [projects]);
      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-4 md:mb-6 gap-4 md:gap-0">
        <div>
          ...
          <p className="text-muted-foreground text-sm md:text-base">
            Explore and join projects aligned with UN Sustainable Development
            Goals
          </p>
        </div>
        <Link
